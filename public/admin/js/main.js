...
(function($) {
    ...
    try {
        $(".updaterole").on('click', function(e) {
            var urtuserid = $(this).attr('id');
            var roleselected = $('#roleselected_' + urtuserid).val()
            var urtname = $('#urtname_' + urtuserid).val()
            var urtcrole = $('#urtcrole_' + urtuserid).val()
            ...
            if (roleselected != "") {
                ...
                swal({
                    title: "Are you sure?",
                    text: "\nOnce assigned you are resposible for this update\n\nName: " + urtname + "\nCurrent Position: " + urtcrole + "\nNew Position: " + roleselected,
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
                        swal("Account is updated!", {
                            icon: "success",
                        }).then(function(isConfirm) {
                            if (isConfirm) {
                                window.location.reload();
                        ...
                        $.ajax({
                            url: "/admin/create",
                            type: "POST",
                            data: { id: urtuserid, action: 'updateadmin', roleselected: roleselected }, //send this to server
                            success: function(returned) {
                                console.log(returned)
                                console.log('aajaxworks'); // here can get the return of route
                                // window.location.reload();
            ...
            swal({
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        url: "/admin/create",
                        type: "POST",
                        data: { id: dtuserid, action: 'deleteadmin' }, //send this to server
                        success: function(returned) {
                            console.log(returned)
                            console.log('aajaxworks'); // here can get the return of route
                            // window.location.reload();
...
(function($) {
    ...
    socket.on('connect', function(data) {
        socket.emit("join", "hellofromclient");
    });
    ...
    socket.on('createnotification', function(data) {
        ...
        if ((data.to).includes(usertype)) {
            var notificationquantity = parseInt($(".nqn").text()) + 1;
            $('.nqn').replaceWith('<span class="quantity nqn">' + notificationquantity + '</span>');
            $('.nqt').replaceWith('<p class="nqt">You have ' + notificationquantity + ' Notifications</p>');
            $('.notifiitem3').remove();
            $('.notifiitem2').addClass('notifiitem3');
    ...
    try {
        $(".createnotification").on('click', function(e) {
            var notificationemail = $(this).attr('id');
            var notificationtarget = $("#notificationtarget").val();
            ...
            swal({
                    title: "Are you sure?",
                    text: "\nOnce sent you are resposible for this notification\n\nFrom: " + notificationemail + "\nTo: " + notificationtarget,
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                .then((willDelete) => {
                    if (willDelete) {
                        swal("Notification sent!", {
                            ...
                            if (isConfirm) {
                                window.location.reload();
                                window.location.href = '#notificationheader';
                            }
                        });
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            dataType: 'json',
                            data: { email: notificationemail, action: 'sendnotification', target: JSON.stringify(notificationtarget), subject: notificationsubject, message: notificationmessage, datetime: datetime }, //send this to server
                            success: function(returned) {
                                console.log(returned)
                                console.log('aajaxworks'); // here can get the return of route
                                // window.location.reload();
...
$(document).ready(function() {
    ...
    $('select.chosen', this).chosen({ width: "100%" });
